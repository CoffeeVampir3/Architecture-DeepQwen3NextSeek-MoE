...
class GatedAttention(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.full_gate_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=False)
    ...
    def reset_parameters(self):
        nn.init.uniform_(self.q_proj.weight, -0.06, 0.06)
        nn.init.uniform_(self.k_proj.weight, -0.06, 0.06)
        nn.init.uniform_(self.v_proj.weight, -0.06, 0.06)
        nn.init.uniform_(self.o_proj.weight, -0.06, 0.06)
    ...
    def _compute_rope_embeddings(self, max_position_embeddings, head_dim, base=10000, dtype=None, device=None):
        inv_freq = 1.0 / (base ** (torch.arange(0, head_dim, 2, device=device).bfloat16() / head_dim))
        t = torch.arange(max_position_embeddings, device=device, dtype=torch.bfloat16)
    ...
    def forward(
        ...
        per_head_gate_scores = gate_scores.view(bsz, seq_len, self.num_heads, self.head_dim)
