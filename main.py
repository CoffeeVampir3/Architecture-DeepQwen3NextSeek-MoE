...
def load_and_preprocess_data(max_length=255):
    dataset = load_dataset("skeskinen/TinyStories-hf", split="train[:5%]") #[:1%]
...
def train(model, train_dataset, tokenizer, num_epochs=10, batch_size=64, learning_rate=4e-5, update_rate=1e-4):
    device = torch.device("cuda")
    model.to(device)

    rmsnorm_weightdecayed = {'params': [p for name, p in model.named_parameters() if 'rmsnorm.weight' in name], 'weight_decay': 1e-4}
    others = {'params': [p for name, p in model.named_parameters() if 'rmsnorm.weight' not in name], 'weight_decay': 3e-4}
